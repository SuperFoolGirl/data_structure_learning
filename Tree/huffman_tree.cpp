...
struct HuffmanNode {
    ...
    // 构造函数 for leaf nodes
    HuffmanNode(char data_char, int freq)
        : data(data_char)
        , frequency(freq)
        , left(nullptr)
        , right(nullptr) {}
...
public:
    // 构造函数：根据文本构建哈夫曼树
    explicit HuffmanTree(const std::string& text)
        : root(nullptr) {
        if (text.empty()) {
    ...
    std::string decode(const std::string& encodedText) const {
        ...
        for (char bit : encodedText) {
            ...
            if (current && current->isLeaf()) {
                ...
                // 路径中断，编码流有问题
                throw std::runtime_error("Error: Invalid Huffman code encountered during decoding "
        ...
        if (current != root) {    // 如果解码结束后没有回到根节点，说明最后一个字符的编码不完整
            throw std::runtime_error("Error: Encoded text is incomplete or "
...
int main() {
    ...
    try {
        ...
        // HuffmanTree empty_tree(empty_text); //
        // 构造函数会打印警告，但不会抛异常
