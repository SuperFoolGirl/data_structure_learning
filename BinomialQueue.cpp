...
public:
    ...
    void deleteMin(const Comparable &minItem)
    {
        ...
        // minIndex是包含最小元素的那棵二项树的高度。我们要把从这棵被删除树拆解下来的所有子树，重新组织到一个临时队列deletedQueue 中。这些子树的高度范围是从0到minIndex - 1，一共minIndex个不同高度的子树。为了能把所有这些不同高度的子树都恰当地存放到deletedQueue的theTrees向量里，向量的大小就需要调整为能容纳这些高度的树，所以大小设为minIndex + 1
        // 注意 是要把这个树拆了 塞到一个新队列。举个例子，minIndex是3（8个节点），就得把8-1=7拆成1+2+4（高度为3的待删二项树有三棵子树，结点数分别为4，2，1；再加上根节点回到8个）
        ...
        // 如果认为单节点树高度为0，那么对于二项树来说，高度为n <==> 节点个数为2^n，n棵子树，每个子树分别引领着高度为n-1到0的树（最左边的最高n-1，最右边的兄弟子树高度为0）
        for (int i = minIndex - 1; i >= 0; i--) // 开始处理根节点的子树 最左边的子树高度比父亲小1 从这开始。i其实正好对应了所有子树的高度
        {
            ...
            deletedQueue.theTrees[i]->nextSibling = nullptr; // 切断兄弟关系。注意 我们是在把一棵树的子树，拆成若干完整的树。子树之间是有nextSibling指针相连的，我们把子树拆成若干完整树后，他们各自都成为了新的根节点。根节点没有兄弟，所以要切断。dQ虽然是新实例化的对象，但已经被赋值为dT了，所以自然继承了dT的属性
        }

        theTrees[minIndex] = nullptr; // 将原队列中对应最小元素所在树的位置清空
        currentSize -= deletedQueue.currentSize + 1; // 更新原队列的元素数量，减去临时队列中的元素数以及被删除的最小元素这一个。curentSize统计的是整个二项队列的节点，即13=8+4+1
    ...
    void merge(BinomialQueue &rhs)
    {
        ...
        for (int i = 0, j = 1; j <= currentSize; i++, j *= 2) // 外层for循环遍历所有可能的树高度，从高度 0 开始，每次高度翻倍（j *= 2），直到覆盖所有可能包含元素的高度。i作为索引对应theTrees向量的位置。因此 每次操作都取高度相同的树（t2存在该高度的树的话），因此可以调用combineTrees函数来处理
        {
...
private:
    ...
    vector<BinomialNode *> theTrees; // 存储所有二项树的根节点，数组索引可以对应二项树的高度，通常，索引为i的位置存放的是高度为i的二项树的根节点。注意 二项队列里不可能存在多棵高度相同的树，原因就是每个数子对应的二进制表示有且仅有一个。13个节点用二项树表示应该是8，4，1 而不可能是4，4，4，1。每当出现相同高度的树，都会即时合并它们
