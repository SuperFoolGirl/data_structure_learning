...
private:
    struct LeftistNode
    {
        ...
        LeftistNode(const Comparable &theElement, LeftistNode *l = nullptr, LeftistNode *r = nullptr, int np = 0) : element(theElement), left(l), right(r), npl(np) {}
    };
    ...
    LeftistNode *merge(LeftistNode *h1, LeftistNode *h2)
    {
        ...
        if (h2 == nullptr)
            return h1;
        // 根据左式堆的堆序性质（在最小左式堆里，父节点元素小于等于子节点元素），比较h1和h2的根节点元素值。如果h1->element < h2->element，调用merge1函数，将h2合并到h1下；否则，把h1合并到h2下，这样能保证合并后的堆依然满足堆序性质。
    ...
    LeftistNode *merge1(LeftistNode *h1, LeftistNode *h2)
    {
        ...
        {
            h1->right = merge(h1->right, h2); // 将h2与h1的右子树进行合并。这利用了递归，不断把小的子堆合并到合适位置，保证合并过程符合堆序性质.如果h1->right=nullptr,那h2就成为h1->right。这里注意是找h1的右子树
