...
private:
    ...
    void buildHeap() {
        // 循环从i = currentSize / 2开始。对于一个完全二叉树（用数组表示的二叉堆通常是完全二叉树），它的非叶子节点编号范围是从1到currentSize /
        // 2。叶子节点不需要进行下滤操作，因为它们没有子节点，天然满足堆的性质。从currentSize / 2 开始，倒序遍历到1，可以覆盖所有有子节点的节点。 加入一共有4层
        // 即15个节点 前三层15/2 = 7 = pow(2,3)-1，最后一层全是叶子节点 且占了另外一半，因为pow(2,3)*2=pow(2,4)（加一减一就不细究了）
        for (int i = currentSize / 2; i > 0; i--) {
            percolateDown(i);    // 下滤充分利用了完全二叉树底层节点多的特性。在完全二叉树里，越靠近底层，节点数量越多 ，下滤能快速把底层不符合堆性质的大元素
                                 // “归位”，使得堆快速成型。相比上滤，性能更好
        }
    }
    ...
    void percolateDown(int hole) {
        ...
        for (; hole * 2 <= currentSize; hole = child)    // 这个判断条件是合理的。如果一个节点有右儿子，根据最小二叉堆的性质
                                                         // 它一定有左儿子，反之则不成立；更新条件，把hole更新为上一个循环里上移的子节点
        {
            // 1. 找到当前节点的左子节点
            child = hole * 2;
